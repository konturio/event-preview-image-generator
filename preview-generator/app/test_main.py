...
async def test_get_screenshot_generates_on_cache_miss(monkeypatch):
    """Ensure cache miss triggers generator and stores the result."""
    ...
    assert result == generated_bytes, 'expected generated screenshot bytes when cache miss occurs'
    assert cache_mock.get.await_count == 1, 'expected cache.get to be awaited exactly once on cache miss'
    assert main.screenshot.await_count == 1, 'expected screenshot generator to run exactly once on cache miss'
    assert cache_mock.set.await_count == 1, 'expected cache.set to be awaited exactly once to store generated bytes'
